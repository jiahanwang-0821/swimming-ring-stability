function figure2_critical_angle_sensitivity
clear; clc; close all;

% ---------- Baseline from IA ----------
baseCritical = 53.1;      % degrees
baseVolume = 0.05428;     % m^3
rhoWater = 1000;
baseDensityRatio = 0.6;

% ---------- Figure ----------
fig = figure('Color','w','Name','Critical Angle Sensitivity Simulator');
...
rhoSlider = makeSlider(0.72,0.28,0.1,1.1,0.6);
...
uicontrol('Style','text','Units','normalized','Position',[0.72 0.33 0.22 0.03],...
    'String','Density Ratio \rho_{ring}/\rho_{water}','BackgroundColor','w');
...
function updatePlot
    ...
    r_current = rSlider.Value;
    ringDensity = rhoSlider.Value;

    ringVol = 2*pi^2*R_current*r_current^2;
    ...
    % ---------- Status ----------
    if ringDensity >= rhoWater
    status = 'Sinks';
elseif thetaDeg < criticalDeg
    status = 'Stable';
else
    status = 'Capsized';
end

    title(ax,'Critical Angle Sensitivity Simulator',...
    ...
    'Status: %s'], ...
    thetaDeg,R_current,r_current,criticalDeg,status);
end
