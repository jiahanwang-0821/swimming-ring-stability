...
Figure 2 (Animation 1). Representative configuration generated by the MATLAB implementation of the Ideal Hydrostatic Model.

The displayed controls vary the geometric and physical input parameters, while the calculated equilibrium height, submerged-volume ratio, righting arm, restoring moment, and stability state are reported as outputs.
...
Unlike the Ideal Hydrostatic Model, this implementation allows the vertical position of the center of gravity to be adjusted while preserving the same hydrostatic formulation. Consequently, stable, neutral, and unstable equilibrium configurations can be explored interactively by varying the center-of-gravity location.

<img width="1231" height="767" alt="Animation_2" src="https://github.com/user-attachments/assets/ac6d325f-7499-448d-aae9-3c09c4c89859" />

Figure 3 (Animation 2). Representative configuration generated by the MATLAB implementation of the General Hydrostatic Stability Model, illustrating an unstable equilibrium produced by an elevated center-of-gravity position.

Together, the two MATLAB implementations provide the computational basis for investigating both the baseline hydrostatic model and its generalized extension. The computational framework described above is verified in the following section before being applied to investigate the hydrostatic restoring behavior and nonlinear dynamic response of the floating torus.
...
Figure 4 shows the computed hydrostatic restoring moment as a function of the prescribed tilt angle for the baseline ideal torus model.

<img width="1338" height="983" alt="figure_2" src="https://github.com/user-attachments/assets/c8ef8e43-3741-47db-a7ed-4075b59b363a" />

（Figure 4: Numerically computed hydrostatic restoring moment as a function of tilt angle）
...
To examine the numerical behavior near the upright equilibrium more clearly, the restoring moment is shown again using an expanded angular scale in Figure 6.

<img width="1234" height="828" alt="figure_4" src="https://github.com/user-attachments/assets/cdbc779b-62c2-4634-beb3-f0eeafcf1722" />

(Figure 6. Restoring moment near the upright equilibrium).
...
Although the restoring-moment curve in Figure 4 appears almost discontinuous near the upright position because of the large plotting range, the enlarged view in Figure 6 confirms that the numerical solution remains continuous and smooth across $\theta=0^\circ$. This indicates that the apparent sharp transition in Figure 4 is a visualization effect rather than a numerical artifact.
